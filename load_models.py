...
def base_predictors(model_path, data_path, hpc, classpath):
    ...
    def preprocessing(jf):
        ...
        for parameters in all_parameters:
            ...
            jf.write('groovy -cp %s %s/load_base_predictors.groovy %s %s %s %s \n' % (classpath, working_dir, data_path,
                                                                                      project_path,
                                                                                      fold,
                                                                                      classifier.split('.')[0]
                                                                                      ))
...
def ensemble(model_path, data_path, ens_model):
    ens_model_path = os.path.join(model_path, 'ens_model.pkl')
    ens_model_dict = pickle.load(open(ens_model_path,'p'))
    data_df = pandas.read_csv(os.path.join(data_path))
    if ens_model == "Mean":
